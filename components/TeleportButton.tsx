...
export const TeleportButton: React.FC<TeleportButtonProps> = ({ agentName, onClick, agents, variant = 'pill' }) => {
    ...
    if (variant === 'inline') {
        return (
            ...
            >
                <span className={`text-base flex items-center justify-center w-6 h-6 ${isActive ? 'scale-110 rotate-[360deg]' : 'grayscale group-hover:grayscale-0'} transition-all duration-700`}>
                    {isActive ? <span className="drop-shadow-lg">✨</span> : renderIcon(agent.icon)}
                </span>
        ...
        <div className="flex flex-col items-center py-8">
            ...
            >
                <div className="flex items-center gap-4">
                    <span className="text-2xl flex items-center justify-center w-8 h-8">{isActive ? <span className="drop-shadow-lg">⚙️</span> : renderIcon(agent.icon)}</span>
