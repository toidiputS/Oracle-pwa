...
import { AgentCard, renderIcon } from './AgentCard';
                                        ...
                                        <div className="flex items-start justify-between mb-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-lg bg-white/5 flex items-center justify-center text-xl border border-white/10 group-hover:border-cyan-500/30 transition-colors shadow-inner">
                                                    {renderIcon(agent.id === 'ORACLE' ? '/icon.svg' : agent.icon)}
                                                </div>
                                                <div>
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{agent.id}</span>
                                                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest truncate">{agent.name}</span>
                                                    </div>
                                                    <h4 className="text-[15px] font-black text-white uppercase tracking-wide group-hover:text-cyan-400 transition-colors truncate max-w-[140px] leading-tight">{agent.role}</h4>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-4">
                                            {agent.toolCard && agent.toolCard.useThisWhen.length > 0 && (
                                                <div className="bg-red-950/20 p-3 rounded-xl border border-red-500/10">
                                                    <span className="text-[7px] text-red-400 font-black uppercase tracking-[0.2em] block mb-1.5">Pain Points</span>
                                                    <ul className="space-y-1">
                                                        {agent.toolCard.useThisWhen.slice(0, 2).map((pain, i) => (
                                                            <li key={i} className="text-[9px] text-red-100 font-bold flex items-start gap-2 leading-tight">
                                                                <span className="text-red-500 mt-0.5 text-[9px] drop-shadow-[0_0_5px_rgba(239,68,68,0.8)] font-black">✕</span> <span className="line-clamp-1 uppercase tracking-wide">{pain}</span>
                                                            </li>
                                                        ))}
                                                        {agent.toolCard.useThisWhen.length > 2 && (
                                                            <li className="text-[8px] text-red-500/80 font-bold italic pl-3 pb-0.5">+ {agent.toolCard.useThisWhen.length - 2} more</li>
                                                        )}
                                                    </ul>
                                                </div>
                                            )}
                    ...
                    <div className="p-6 sm:p-10">
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <div className="flex items-center gap-3 mb-1">
                                        ...
                                        <span className="text-xs sm:text-sm text-white/80 font-bold uppercase tracking-[0.4em] italic block">{agent.name}</span>
                                    </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                            <div className="space-y-8">
                                <section>
                                    <h4 className="text-[10px] font-black text-red-500 uppercase tracking-[0.4em] mb-4 flex items-center gap-2">
                                        <span className="w-2 h-2 rounded-full bg-red-400 animate-pulse"></span> 01_PAIN_POINTS
                                    </h4>
                                    <ul className="list-none space-y-3">
                                        {(agent.toolCard?.useThisWhen || []).map((t, i) => (
                                            <li key={i} className="text-lg text-red-50 font-black flex items-start gap-4 uppercase tracking-widest">
                                                <span className="text-red-500 mt-1 text-xl drop-shadow-[0_0_8px_rgba(239,68,68,0.8)]">✕</span> {t}
                                            </li>
                                        ))}
                                    </ul>
                                </section>
                                ...
                                <section>
                                    <h4 className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.4em] mb-4 flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-cyan-400"></span> 02_MISSION_PURPOSE
                                    </h4>
                                    {agent.oracleInsight && (
                                        <p className="text-cyan-50 text-xl font-black italic mb-4">"{agent.oracleInsight}"</p>
                                    )}
                                ...
                                <section>
                                    <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mb-4 flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span> 03_CONSTRAINTS
                                    </h4>
                                    <div className="space-y-3">
                                        <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                                            <span className="text-[9px] text-orange-400 font-black uppercase tracking-widest block mb-2">Abstain_When:</span>
                                            <ul className="list-none space-y-2">
                                                {(agent.toolCard?.doNotUseWhen || []).map((t, i) => (
                                                    <li key={i} className="text-sm text-slate-400 flex items-center gap-3">
                                                        <span className="text-orange-500 font-bold">⚠</span> {t}
                                                    </li>
                                                ))}
                                            </ul>
                                        </div>
                            ...
                            <div className="space-y-8">
                                <section>
                                    <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mb-4 flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-amber-500"></span> 04_INPUT_INVENTORY
                                    </h4>
                                ...
                                <section>
                                    <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mb-4 flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> 05_DELIVERABLES
                                    </h4>
